...
if(!("friends" %in% dir("./twitterdata"))) {
    dir.create("./twitterdata/friends")
}

## users to get
users <- setdiff(users, dir("./twitterdata/friends/"))
cat(paste("Number of users to get:", length(users), "\n"))

## write followers as csv file
friend_to_csv(users, "./twitterdata/friends/", sleep = 60)
